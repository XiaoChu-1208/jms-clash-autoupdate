...
机场（尤其 JustMySocks）的节点 IP 会不定期轮换，有时还会把某个节点在 **SS ↔ VMess** 之间切换、下架或新增。Clash Verge 自带的「订阅更新」对**裸节点列表订阅**支持不好（会冲掉你手配的分组/规则），而「只换 IP 的脚本」遇到协议切换会把节点改成死的。

本项目用**结构化同步**解决：按「节点稳定标识」对齐后整条节点按订阅重建（协议正确），只替换你分组里的节点成员，**你的分组结构 / 规则 / DNS 原样保留**，改完走 mihomo 的 unix socket **热重载**立即生效。
...
- 🏷️ **节点友好名**：把 `JMS-xxx@c70s1...` 这种裸名换成你定的名字（如 `🇺🇸 洛杉矶 1`）。
- 📍 **可只留某地区**：`only_locations: ["洛杉矶"]` 就只保留洛杉矶节点，保持出口 IP 纯净。
- ⏱️ **三重自动触发**：开机/登录 + 每 N 分钟定时 + **开盖/唤醒**（可选）。
- ♻️ **空跑零成本**：节点没变化时不写盘、不重载，所以可以高频跑。
...
```yaml
# 必填：订阅链接（⚠️ 你的付费账号密钥，别公开、别提交进 git）
